...
def build_chunk_prompt(url: str, title: str, focus: Optional[str], chunk_index: int, chunk_text: str) -> str:
    parts = [
        ...
        "Then include at most 4 bullets with facts, labels, headings, or details that matter to the user focus.",
        "Ignore navigation, repeated chrome, and unrelated boilerplate unless directly relevant.",
        f"Source URL: {url}",
        f"Page title: {title}",
...
def build_final_prompt(url: str, title: str, focus: Optional[str], chunk_summaries: List[str]) -> str:
    ...
    parts = [
        ...
        "Prefer concrete details that match the user focus over generic page copy.",
        "If the focus is specific, center the answer on matching details and omit unrelated sections.",
        f"Source URL: {url}",
        f"Page title: {title}",
    ]
    if focus:
...
def fallback_output(url: str, title: str, focus: Optional[str], cleaned_text: str, reason: str) -> str:
    chunks = build_semantic_chunks(cleaned_text)
    focused_excerpt = "\n\n".join(chunks)
    parts = [
        f"TITLE: {title}",
        f"SOURCE: {url}",
...
def focused_extract_output(url: str, title: str, focus: Optional[str], cleaned_text: str) -> str:
    chunks = build_semantic_chunks(cleaned_text)
    focused_excerpt = clip_utf8("\n\n".join(chunks), MAX_FINAL_CONTEXT_BYTES)
    parts = [
        f"TITLE: {title}",
        f"SOURCE: {url}",
        "NOTE: Webpage extract in document order. Use the focus as guidance when summarizing this content.",
    ]
    if focus:
        parts.append(f"FOCUS: {focus}")
    parts.extend(["CONTENT:", focused_excerpt])
...
def fetch_extract_url_input(input_text: str) -> str:
    url, focus = parse_input_text(input_text)
    html_text = fetch_url(url)
    title, cleaned_text = extract_readable_content(url, html_text)
    return focused_extract_output(url, title, focus, cleaned_text)
...
def cli_mode(argv: List[str]) -> int:
    ...
    try:
        output = summarize_url_input(input_text) if summarize else fetch_extract_url_input(input_text)
        print(output)
        return 0
    except RuntimeError as exc:
